...
echo -e "\e[34m-->> Disable the mysql default repo. \e[0m" | tee -a ${log}
dnf module disable mysql -y  &>> ${log}
func_exit_status

# Setup the MySQL5.7 repo file
echo -e "\e[34m-->> Configuring mysql5.7 repository repo. \e[0m" | tee -a ${log}
cp mysql.repo /etc/yum.repos.d/mysql.repo    &>> ${log}
func_exit_status

#Install MySQL Server
echo -e "\e[34m-->> Installing the mysql-community-server package. \e[0m" | tee -a ${log}
dnf install mysql-community-server bash-completion -y     &>> ${log}
func_exit_status

# Start MySQL Service
echo -e "\e[34m-->> Starting & Enabling the mysqld service. \e[0m" | tee -a ${log}
...
#Password = RoboShop@1
echo -e "\e[34m-->> Configuring the MYSQl root password. \e[0m" | tee -a ${log}
